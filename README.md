...
- **v2 — Predict cost impact of a Terraform PR before merge.** Read `terraform show -json plan.tfplan`, price every changing resource (AWS via the live Pricing API; GCP and Azure via embedded static price tables), and post (or upsert) a Markdown comment on the PR with the net monthly delta, top movers, and a 1–3 sentence LLM narrative. Ships as a GitHub Action and as the `oracle pr-check` subcommand. See **[docs/v2-guide.md](docs/v2-guide.md)**.
- **v3 — Insights Agent.** Polyglot Go + Python extension adding agentic FinOps analysis on top of v1/v2 cost data — a hand-rolled LangGraph supervisor over specialist agents, RAG over a FinOps corpus (pgvector), production guardrails, real billing across all three clouds (AWS Cost Explorer, GCP BigQuery export, Azure Cost Management), optional multi-turn conversational memory (LangGraph PostgresSaver), and a CLI + HTTP surface with an in-dashboard chat widget. See **[v3 — Insights Agent](#v3--insights-agent)** below, **[docs/v3-guide.md](docs/v3-guide.md)**, and **[insights-agent/README.md](insights-agent/README.md)**.
...
- [X]  **Milestone 8.7** — Real billing integration behind a `billing.Source` abstraction: the v1 cost endpoints now consume normalized cost records, with the snapshot approximation as the default source and an **AWS Cost Explorer** source (real unblended cost, `data_source: billing_aws_cost_explorer`) selectable via `CLOUDORACLE_BILLING_PROVIDER=aws_cost_explorer`, plus a **GCP BigQuery billing-export** source (real net cost, `data_source: billing_gcp_bigquery`) selectable via `CLOUDORACLE_BILLING_PROVIDER=gcp_bigquery`, and an **Azure Cost Management** source (real actual cost, `data_source: billing_azure_cost_management`) selectable via `CLOUDORACLE_BILLING_PROVIDER=azure_cost_management` — closing real billing across all three clouds behind one interface
- [X]  **Multi-turn conversational memory** — `POST /ask` takes an optional `thread_id`; when `DATABASE_URL` is set the supervisor graph is compiled with LangGraph's `AsyncPostgresSaver`, so a thread's prior messages carry into follow-ups ("and the month before?"). Per-run accounting resets each turn so the run-wide caps aren't tripped by thread history. No DB configured keeps single-shot behavior
- [X]  **In-dashboard chat** — a React chat widget on the web dashboard talks to the agent's `/ask` with a per-conversation `thread_id`, surfacing the whole insights-agent to dashboard users
...
- [X]  **Azure pricing** — `azurerm_linux_virtual_machine` (+ OS disk, Spot upper-bound) and `azurerm_managed_disk`, priced from an embedded static table (`internal/pricing/azure_prices.json`, East US base + region multipliers), same deterministic/credential-free/`medium`-confidence approach as GCP (the Azure Retail Prices API is the documented upgrade path). `azurerm_*` types dispatch through `internal/iac/azure` + `internal/pricing/azure_*.go`
