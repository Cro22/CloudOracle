...
type AzureProvider struct {
	vms            azureVMLister
	disks          azureDisksLister
	sql            azureSQLLister
	webApps        azureWebAppsLister
...
func NewAzureProvider(ctx context.Context, cfg config.Config) (*AzureProvider, error) {
	...
	return &AzureProvider{
		vms:            &realAzureVMLister{client: vmClient},
		disks:          &realAzureDisksLister{client: disksClient},
		sql:            &realAzureSQLLister{servers: sqlServersClient, databases: sqlDBClient},
		webApps:        &realAzureWebAppsLister{client: webAppsClient},
...
func (p *AzureProvider) fetchSQLDatabases(ctx context.Context) ([]shared.Resource, error) {
	dbs, err := p.sql.listSQLDatabases(ctx)
	if err != nil {
		return nil, fmt.Errorf("listing Azure SQL databases: %w", err)
	}
...
func (p *AzureProvider) fetchFunctionApps(ctx context.Context) ([]shared.Resource, error) {
	apps, err := p.webApps.listWebApps(ctx)
	if err != nil {
	...
	for _, app := range apps {
		// Web Apps and Function Apps live in the same collection — the Kind
		// field is what distinguishes them. Skip non-function entries.
